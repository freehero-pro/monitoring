...
- Аккаунт [Resend](https://resend.com) с подтверждённым доменом: с него уходят письма
  со ссылками для входа. Без него ссылка печатается в лог, и войти сможете только вы.
...
EMAIL_PROVIDER=resend                        # console — ссылка в лог, resend — письмо
EMAIL_FROM=monitoring@company.ru             # домен должен быть подтверждён в Resend
...
Если письмо не пришло, ссылку можно взять из лога — при `EMAIL_PROVIDER=console` она
пишется туда с предупреждением:
...
|---|---|
| Письмо не приходит | `docker compose logs app \| grep -i resend` — там код ответа и текст ошибки. Частая причина: домен `EMAIL_FROM` не подтверждён в Resend |
| Нужно войти без почты | Поставьте `EMAIL_PROVIDER=console`, `up -d app` — ссылка появится в логе |
...
пароли и ключ Resend через Secret. Отдельная джоба для миграций не нужна, приложение применяет их
при старте.
